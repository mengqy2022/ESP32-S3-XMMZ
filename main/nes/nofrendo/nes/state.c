...
int state_load(const char* fn)
{
   ...
   MESSAGE_INFO("state_load: file '%s' opened, blocks=%lu.\n", fn, (unsigned long)numberOfBlocks);
   ...
   for (uint32 blk = 0; blk < numberOfBlocks; blk++)
   {
      ...
      {
         MESSAGE_INFO("  - Found base block (%lu bytes)\n", (unsigned long)blockLength);
      ...
      {
         MESSAGE_INFO("  - Found VRAM block (%lu bytes)\n", (unsigned long)blockLength);
      ...
      {
         MESSAGE_INFO("  - Found SRAM block (%lu bytes)\n", (unsigned long)blockLength);
      ...
      {
         MESSAGE_INFO("  - Found mapper block (%lu bytes)\n", (unsigned long)blockLength);
      ...
      {
         MESSAGE_INFO("  - Found sound block (%lu bytes)\n", (unsigned long)blockLength);
      ...
      {
         MESSAGE_INFO("  - Found info block (%lu bytes)\n", (unsigned long)blockLength);
